...
# Conservative Yahoo pacing: <=100 Yahoo operations/minute.
...
def retry_yahoo(fn, label, attempts=6):
    """Pace Yahoo calls and retry throttling/transient failures with exponential backoff."""
    last=None
...
def universe():
    ...
    for u in urls:
        ...
        frames.append(pd.read_csv(io.StringIO(txt),sep="|"))
    a,b=frames
...
def fundamentals(tickers):
    ...
    for i,t in enumerate(tickers,1):
        if t in done:
            rows.append(done[t]); continue
        def get_info(): return yf.Ticker(t).info
        info=retry_yahoo(get_info, f"fundamentals {t}")
        row={"ticker":t}
        if info:
...
def prices(tickers):
    """Fetch price history in small serial batches; retry failed batches ticker-by-ticker."""
    rows=[]
    batch_size=20
    for start in range(0,len(tickers),batch_size):
        batch=tickers[start:start+batch_size]
        def dl():
            return yf.download(batch,period="1y",interval="1d",group_by="column",auto_adjust=True,threads=False,progress=False,timeout=30)
        data=retry_yahoo(dl, f"prices batch {start//batch_size+1}")
        if data is None or data.empty:
            print(f"Skipping failed price batch after retries: {batch}", flush=True); continue
        try:
        ...
        for t in batch:
            try:
                ...
                print(f"Price metric skip {t}: {exc}", flush=True)
...
def score(df):
    ...
    df["best_lens"]=df[list(LENSES)].max(axis=1); df["lens_winner"]=df[list(LENSES)].idxmax(axis=1)
...
def main():
    ...
    required_price={"ticker","price","dollar_volume"}
    if p.empty or not required_price.issubset(p.columns): raise RuntimeError("Price stage produced insufficient schema; refusing incomplete FINAL screen")
    ...
    # Never let a missing ticker crash the run; missing market cap remains excluded from scoring and is auditable.
    ...
    s=score(e).sort_values("final_quant_score",ascending=False); s.to_csv(OUT/"04_all_scored.csv",index=False)
    semis=pd.concat([s.nlargest(40,l) for l in LENSES]).drop_duplicates("ticker").sort_values("final_quant_score",ascending=False); semis.to_csv(OUT/"05_semifinalists.csv",index=False)
    ...
    audit={"starting_universe":len(u),"price_eligible":len(p),"investable_before_marketcap":len(u.merge(p,on='ticker').query('price>=3 and dollar_volume>=2000000')),"fundamentals_requested":len(f),"market_cap_missing":missing_mc,"fully_scored":len(s),"semifinalists":len(semis),"quant_finalists":len(finalists),"yahoo_rate_cap_per_minute":100,"three_lenses":list(LENSES),"FINAL_LABEL_ALLOWED":False,"note":"Top candidates require fresh earnings/guidance, revisions, catalyst, downside, and binary-risk verification before FINAL ranking."}
