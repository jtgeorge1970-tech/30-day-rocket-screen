...
def test_notification_delivery_can_never_stop_engine4_invocation():
    for filename in ("engine4-early-wake.yml","engine4-production.yml","engine4-manual-on-demand.yml","engine4-watchdog.yml"):
...
def test_acceptance_replay_cannot_suppress_live_cycle():
    manual=(ROOT/".github/workflows/engine4-manual-on-demand.yml").read_text()
    watchdog=(ROOT/".github/workflows/engine4-watchdog.yml").read_text()
    production=(ROOT/".github/workflows/engine4-production.yml").read_text()
    assert "mode=live_today" in manual
    assert "mode=live_today" in watchdog
    assert "ACCEPTANCE_DATE_ET" not in production
    assert 'ENGINE_DATE_ET=$(TZ=America/New_York date +%F)' in production

def test_primary_result_notification_cannot_claim_full_completion(tmp_path, monkeypatch):
    monkeypatch.setattr(engine4_notify,"OUT",tmp_path)
    (tmp_path/"final_signal.json").write_text('{"status":"NO_TRADE","ticker":"MARKET","message":"No primary trade."}')
    result=engine4_notify.notify("final",dry_run=True)
    text=result["notification_text"]
    assert "full Engine 4 run is NOT complete" in text
    assert "ENGINE 4 FINAL: NO_TRADE" in text
    completed=engine4_notify.notify("complete","All stages finished.",dry_run=True)
    assert completed["notification_text"].startswith("ENGINE 4 COMPLETE:")
